...
class User {
  ...
  async getByFiltered(type, value, inputs) {
    try {
      ...
      console.log(`SELECT ${inputs} FROM public."User" WHERE ${type} = ${value}`);
      result = await db.any(`SELECT $1:name FROM public."User" WHERE $2:name = $3`, [
        inputs,
        type,
        value,
      ]);
  ...
  async exists(type, value) {
    try {
      ...
      result = await db.any(`SELECT exists(SELECT from public."User" WHERE $1:name = $2);`, [
        type,
        value,
      ]);
